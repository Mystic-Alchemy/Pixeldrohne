...
async def on_message(message):
    # command existiert?
    if message.content.lower().startswith('p.') and message.content.lower().split(" ")[0] not in pxldrn.adv.config.lists.commands:
        await client.send_message(message.channel, embed=pxldrn.adv.embed_data.help_embeds.no_command(client))
        return
    # Spielstatus
    elif message.content.startswith('p.game') and message.author.id == keys.pmcid:
    ...
    # Pixels Liebling
    elif message.content.startswith('p.pixels_liebling'):
    ...
    # Hilfe
        ...
        await client.send_message(message.author, embed=pxldrn.hilfe.hilfe(message.content[7:].lower(), len(message.content), client))

    # Sysinfo
    elif message.content.lower().startswith('p.sysinfo'):
        await client.send_message(message.channel, embed=pxldrn.adv.embed_data.system_info())

    # Python help
    elif message.content.lower().startswith('p.python'):
    ...
    # Bot Remote
    elif message.content.startswith('p.say') and message.channel == client.get_channel('347000675093315584'):
    ...
    # Bot Remote Embed
    elif message.content.startswith('p.esay') and message.channel == client.get_channel('347000675093315584'):
    ...
    # eine "Über" Sektion
    elif message.content.startswith('p.about'):
        await client.send_message(message.channel, embed=pxldrn.adv.embed_data.about())

    # Invite zur Heimat
    elif message.content.lower().startswith('p.test'):
    ...
    # Schere Stein Papier
    elif message.content.lower().startswith("p.ssp"):
    ...
    # Coinflip
    elif message.content.lower().startswith('p.coin'):
        await client.add_reaction(message, pxldrn.adv.coin())

    # Roulette
    # TODO: Einsatz adden!
    elif message.content.lower().startswith("p.roulette"):
    ...
    # Massenlöschung
    elif message.content.lower().startswith('p.purge') and (keys.pmcid or keys.pxlid):
            try:
    ...
    # Botstop
    elif message.content.lower().startswith('p.halt') and message.author.id == keys.pmcid:
        await client.logout()
...
# Ab hier beginnt der Musik-Bot
    ...
    elif message.content.startswith('p.leave'):
        try:
            voice_client = client.voice_client_in(message.server)
            await voice_client.disconnect()
        except AttributeError:
    ...
    elif message.content.lower().startswith('p.radio'):
    ...
    elif message.content.startswith('p.pause'):
        try:
            players[message.server.id].pause()
        except Exception as error:
            await client.send_message(message.channel, "Ein Error ist aufgetreten:\n ```{error}```".format(error=error))

    elif message.content.startswith('p.resume'):
        try:
    ...
    elif message.content.lower().startswith('p.mute'):
